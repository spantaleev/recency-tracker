...
Tracker.prototype.publish = function (resource, version) {
	resource = ('' + resource);
	version = ('' + version);

	console.log('Received publish request for resource ' + resource + ', version: ' + version);

	this.resourceVersions[resource] = version;
	this.persister.set(resource, version);
...
Tracker.prototype.catchUpClient = function (client, resource) {
	var clientVersion = client.resources[resource],
		self = this,
		onLatestVersionDetermined = function () {
			var latestVersion = self.resourceVersions[resource];
			...
			client.events.emit('outdated', resource, latestVersion);
		};

	if (typeof(this.resourceVersions[resource]) !== 'undefined') {
		console.log('Using local version information for: ' + resource);
		onLatestVersionDetermined();
	} else {
		console.log('Loading version information from persister for: ' + resource);
		this.persister.get(resource, function (version) {
			self.resourceVersions[resource] = version;
			onLatestVersionDetermined();
		});
	}
};
